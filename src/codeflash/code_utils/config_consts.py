...
MIN_THROUGHPUT_IMPROVEMENT_THRESHOLD = (
    0.10  # 10% minimum improvement for async throughput
)
...
REFINED_CANDIDATE_RANKING_WEIGHTS = (
    2,
    1,
)  # (runtime, diff), runtime is more important than diff by a factor of 2

# setting this value to 1 will disable repair if there is at least one correct candidate
MIN_CORRECT_CANDIDATES = 2

TOTAL_LOOPING_TIME_EFFECTIVE = TOTAL_LOOPING_TIME
...
EFFORT_VALUES: dict[str, dict[EffortLevel, Any]] = {
    EffortKeys.N_OPTIMIZER_CANDIDATES.value: {
        EffortLevel.LOW: 3,
        EffortLevel.MEDIUM: 5,
        EffortLevel.HIGH: 6,
    },
    EffortKeys.N_OPTIMIZER_LP_CANDIDATES.value: {
        EffortLevel.LOW: 4,
        EffortLevel.MEDIUM: 6,
        EffortLevel.HIGH: 7,
    },
    # we don't use effort with generated tests for now
    EffortKeys.N_GENERATED_TESTS.value: {
        EffortLevel.LOW: 2,
    ...
    EffortKeys.MAX_CODE_REPAIRS_PER_TRACE.value: {
        EffortLevel.LOW: 2,
    ...
    EffortKeys.TOP_VALID_CANDIDATES_FOR_REFINEMENT: {
        EffortLevel.LOW: 2,
        EffortLevel.MEDIUM: 3,
        EffortLevel.HIGH: 4,
    },
    # max number of adaptive optimization calls to make per a single candidates tree
    EffortKeys.ADAPTIVE_OPTIMIZATION_THRESHOLD.value: {
        EffortLevel.LOW: 0,
