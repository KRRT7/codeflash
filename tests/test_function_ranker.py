...
def test_rank_functions(function_ranker, workload_functions):
    ...
    assert (
        len(ranked_functions) > 0
    )  # At least some functions should pass the threshold
    ...
    for i in range(len(ranked_functions) - 1):
        current_time = function_ranker.get_function_addressable_time(
            ranked_functions[i]
        )
        next_time = function_ranker.get_function_addressable_time(
            ranked_functions[i + 1]
        )
...
def test_importance_calculation(function_ranker):
    total_program_time = sum(
        s["own_time_ns"]
        for s in function_ranker._function_stats.values()
