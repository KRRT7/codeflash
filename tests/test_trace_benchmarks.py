...
def test_trace_benchmarks() -> None:
    ...
    project_root = Path(__file__).parent / "code_to_optimize"
    ...
    try:
        ...
        assert len(function_calls) == 8, (
            f"Expected 8 function calls, but got {len(function_calls)}"
        )

        bubble_sort_path = (project_root / "bubble_sort_codeflash_trace.py").as_posix()
        process_and_bubble_sort_path = (
            project_root / "process_and_bubble_sort_codeflash_trace.py"
        ).as_posix()
        ...
        expected_calls = [
            (
                "sorter",
                "Sorter",
                "tests.code_to_optimize.bubble_sort_codeflash_trace",
                f"{bubble_sort_path}",
                "test_class_sort",
            ...
            (
                "compute_and_sort",
                "",
                "tests.code_to_optimize.process_and_bubble_sort_codeflash_trace",
...
from tests.code_to_optimize.bubble_sort_codeflash_trace import \\
    Sorter as tests_code_to_optimize_bubble_sort_codeflash_trace_Sorter
from tests.code_to_optimize.bubble_sort_codeflash_trace import \\
    sorter as tests_code_to_optimize_bubble_sort_codeflash_trace_sorter
...
def test_tests_code_to_optimize_bubble_sort_codeflash_trace_sorter_test_sort():
    for args_pkl, kwargs_pkl in get_next_arg_and_return(trace_file=trace_file_path, benchmark_function_name="test_sort", function_name="sorter", file_path=r"{bubble_sort_path}", num_to_get=100):
        ...
        ret = tests_code_to_optimize_bubble_sort_codeflash_trace_sorter(*args, **kwargs)

def test_tests_code_to_optimize_bubble_sort_codeflash_trace_Sorter_sorter_test_class_sort():
    for args_pkl, kwargs_pkl in get_next_arg_and_return(trace_file=trace_file_path, benchmark_function_name="test_class_sort", function_name="sorter", file_path=r"{bubble_sort_path}", class_name="Sorter", num_to_get=100):
        ...
        if function_name == "__init__":
            ret = tests_code_to_optimize_bubble_sort_codeflash_trace_Sorter(*args[1:], **kwargs)
        else:
            ret = tests_code_to_optimize_bubble_sort_codeflash_trace_Sorter.sorter(*args, **kwargs)

def test_tests_code_to_optimize_bubble_sort_codeflash_trace_Sorter_sort_class_test_class_sort2():
    for args_pkl, kwargs_pkl in get_next_arg_and_return(trace_file=trace_file_path, benchmark_function_name="test_class_sort2", function_name="sort_class", file_path=r"{bubble_sort_path}", class_name="Sorter", num_to_get=100):
        ...
        if not args:
            raise ValueError("No arguments provided for the method.")
        ret = tests_code_to_optimize_bubble_sort_codeflash_trace_Sorter.sort_class(*args[1:], **kwargs)

def test_tests_code_to_optimize_bubble_sort_codeflash_trace_Sorter_sort_static_test_class_sort3():
    for args_pkl, kwargs_pkl in get_next_arg_and_return(trace_file=trace_file_path, benchmark_function_name="test_class_sort3", function_name="sort_static", file_path=r"{bubble_sort_path}", class_name="Sorter", num_to_get=100):
        args = pickle.loads(args_pkl)
        kwargs = pickle.loads(kwargs_pkl)
        ret = tests_code_to_optimize_bubble_sort_codeflash_trace_Sorter.sort_static(*args, **kwargs)

def test_tests_code_to_optimize_bubble_sort_codeflash_trace_Sorter___init___test_class_sort4():
...
"""
        assert (
            test_class_sort_path.read_text("utf-8").strip()
            == test_class_sort_code.strip()
        )
...
from tests.code_to_optimize.bubble_sort_codeflash_trace import \\
    sorter as tests_code_to_optimize_bubble_sort_codeflash_trace_sorter
from tests.code_to_optimize.process_and_bubble_sort_codeflash_trace import \\
    compute_and_sort as \\
    tests_code_to_optimize_process_and_bubble_sort_codeflash_trace_compute_and_sort

functions = ['compute_and_sort', 'sorter']
trace_file_path = r"{output_file.as_posix()}"

def test_tests_code_to_optimize_process_and_bubble_sort_codeflash_trace_compute_and_sort_test_compute_and_sort():
    for args_pkl, kwargs_pkl in get_next_arg_and_return(trace_file=trace_file_path, benchmark_function_name="test_compute_and_sort", function_name="compute_and_sort", file_path=r"{process_and_bubble_sort_path}", num_to_get=100):
        args = pickle.loads(args_pkl)
        kwargs = pickle.loads(kwargs_pkl)
        ret = tests_code_to_optimize_process_and_bubble_sort_codeflash_trace_compute_and_sort(*args, **kwargs)

def test_tests_code_to_optimize_bubble_sort_codeflash_trace_sorter_test_no_func():
...
def test_trace_multithreaded_benchmark() -> None:
    ...
    try:
        ...
        assert len(function_calls) == 10, (
            f"Expected 10 function calls, but got {len(function_calls)}"
        )
        ...
        assert (
            "tests.code_to_optimize.bubble_sort_codeflash_trace.sorter"
