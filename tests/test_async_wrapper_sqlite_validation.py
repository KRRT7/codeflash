...
class TestAsyncWrapperSQLiteValidation:
    ...
    async def test_behavior_async_basic_function(self, test_env_setup, temp_db_path):
        ...
        assert iteration_id.startswith("simple_async_add_") and iteration_id.endswith(
            "_0"
        )
        assert runtime > 0
    ...
    async def test_complex_async_function_with_kwargs(
        ...
        async def complex_async_func(
            pos_arg: str, *args: int, keyword_arg: str = "default", **kwargs: str
        ) -> dict:
            await asyncio.sleep(0.001)
            return {
                "pos_arg": pos_arg,
        ...
        result = await complex_async_func(
            "hello", 1, 2, 3, keyword_arg="custom", extra1="value1", extra2="value2"
        )
