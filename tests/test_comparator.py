    ...
    am = pd.DataFrame(
        {
            "a": [
                datetime.datetime(2020, 2, 2, 2, 2, 2),
                datetime.datetime(2020, 2, 2, 2, 2, 3),
            ],
            "b": [4, 5],
        }
    )
...
def test_xarray():
    ...
    u = xr.Dataset(
        {"temp": (["x", "y"], [[1, 2], [3, 4]])}, coords={"x": [0, 1], "y": [0, 1]}
    )
    v = xr.Dataset(
        {"temp": (["x", "y"], [[1, 2], [3, 4]])}, coords={"x": [0, 1], "y": [0, 1]}
    )
    w = xr.Dataset(
        {"temp": (["x", "y"], [[1, 2], [3, 4]])}, coords={"x": [0, 2], "y": [0, 1]}
    )
    assert comparator(u, v)
    assert not comparator(u, w)
...
def test_attrs():
    ...
    assert comparator(
        obj1, obj2
    )  # Should be equal despite different timestamp/debug_info
...
def test_tensorflow_sparse_tensor() -> None:
    ...
    f = tf.SparseTensor(
        indices=tf.zeros([0, 2], dtype=tf.int64), values=[], dense_shape=[3, 4]
    )
    g = tf.SparseTensor(
        indices=tf.zeros([0, 2], dtype=tf.int64), values=[], dense_shape=[3, 4]
    )
