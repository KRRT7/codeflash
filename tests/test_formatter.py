...
def foo():
    ...
    try:
        new_code = format_code(
            formatter_cmds=["exit 1"], path=temp_file, exit_on_failure=False
        )
        assert new_code == original_code
    except Exception as e:
        assert False, (
            f"Shouldn't throw an exception even if the formatter is not found: {e}"
        )


def _run_formatting_test(
    source_code: str,
    should_content_change: bool,
    expected=None,
    ...
    with tempfile.TemporaryDirectory() as test_dir_str:
        ...
        function_to_optimize = FunctionToOptimize(
            function_name="process_data", parents=[], file_path=target_path
        )
...
"""
    ...
    _run_formatting_test(
        source_code, True, optimized_function="def func2():a=1;b=2;return a+b"
    )
...
'''
    ...
    optimization_function = """def process(self,data):
        ...
        return{'result':[item for item in data if self._is_valid(item)]}"""
    _run_formatting_test(
        source_code, True, optimized_function=optimization_function, expected=expected
    )
