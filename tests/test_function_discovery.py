...
def test_function_eligible_for_optimization() -> None:
    ...
    assert (
        functions_found[file_path][0].function_name
...
def non_classmethod_function(cls, name):
    ...
    """
        ...
        assert inspect_top_level_functions_or_methods(
            file_path, "functionC", class_name="A"
        ).is_top_level
        assert not inspect_top_level_functions_or_methods(
            file_path, "functionD", class_name="A"
        ).is_top_level
        assert not inspect_top_level_functions_or_methods(
            file_path, "functionF", class_name="E"
        ).is_top_level
        assert not inspect_top_level_functions_or_methods(
            file_path, "functionA"
        ).has_args
...
def test_filter_files_optimized():
    ...
    assert not filter_files_optimized(
        file_path_test, tests_root, ignore_paths, module_root
    )
    assert filter_files_optimized(
        file_path_same_level, tests_root, ignore_paths, module_root
    )
    assert filter_files_optimized(
        file_path_different_level, tests_root, ignore_paths, module_root
    )
    assert not filter_files_optimized(
        file_path_above_level, tests_root, ignore_paths, module_root
    )
...
"""
        ...
        modified_functions_test = {
            test_file_path: discovered_test_file.get(test_file_path, [])
        }
        ...
        modified_functions_ignored = {
            ignored_file_path: discovered_ignored.get(ignored_file_path, [])
        }
        ...
        with unittest.mock.patch(
            ...
            modified_functions_submodule = {
                submodule_file_path: discovered_submodule.get(submodule_file_path, [])
            }
        ...
        with unittest.mock.patch(
            ...
            modified_functions_site_package = {
                site_package_file_path: discovered_site_package.get(
                    site_package_file_path, []
                )
            }
        ...
        try:
            ...
            discovered_outside_module = find_all_functions_in_file(
                outside_module_root_path
            )
            modified_functions_outside_module = {
                outside_module_root_path: discovered_outside_module.get(
                    outside_module_root_path, []
                )
            }
        ...
        modified_functions_invalid_module = {
            invalid_module_file_path: discovered_invalid_module.get(
                invalid_module_file_path, []
            )
        }
        ...
        with unittest.mock.patch(
            ...
            assert "propagate_attributes" not in [
                fn.function_name for fn in filtered_funcs.get(original_file_path, [])
            ]
            assert count == 2
