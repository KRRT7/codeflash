...
class TestReadApiKeyFromShellConfig(unittest.TestCase):
        ...
        with patch(
            ...
            if os.name != "nt":
                ...
                with patch(
                    "builtins.open",
                    mock_open(
                        read_data=f"#export CODEFLASH_API_KEY='{self.api_key}'\n"
                    ),
                ) as mock_file:
                ...
                with patch(
                    "builtins.open",
                    mock_open(read_data=f"REM set CODEFLASH_API_KEY={self.api_key}\n"),
                ) as mock_file:
                    self.assertEqual(read_api_key_from_shell_config(), None)
                    mock_file.assert_called_once_with(
                        self.test_rc_path, encoding="utf8"
    ...
    def test_no_api_key(self, mock_get_shell_rc_path):
        ...
        mock_get_shell_rc_path.return_value = self.test_rc_path
        with patch(
            "builtins.open", mock_open(read_data="# No API key here\n")
        ) as mock_file:
    ...
    def test_malformed_api_key_export(self, mock_get_shell_rc_path):
        ...
        if os.name == "nt":
            with patch(
                "builtins.open", mock_open(read_data=f"set API_KEY={self.api_key}\n")
            ):
                result = read_api_key_from_shell_config()
                self.assertIsNone(result)
            with patch(
                "builtins.open",
                mock_open(read_data=f"CODEFLASH_API_KEY={self.api_key}\n"),
            ):
                result = read_api_key_from_shell_config()
                self.assertIsNone(result)
            with patch(
                "builtins.open",
                mock_open(read_data=f"set CODEFLASH_API_KEY=sk-{self.api_key}\n"),
            ):
                result = read_api_key_from_shell_config()
                self.assertIsNone(result)
        else:
            with patch(
                "builtins.open", mock_open(read_data=f"export API_KEY={self.api_key}\n")
            ):
                result = read_api_key_from_shell_config()
                self.assertIsNone(result)
            with patch(
